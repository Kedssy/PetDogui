...
public class EnderecoService {
    ...
    private void validaEndereco(Endereco endereco) throws Exception {
        if (endereco.getNrCep() == null || endereco.getNrCep().isEmpty()) {
            throw new Exception("Cep do endereço não informado");
        }

        if (endereco.getNomeRua() == null || endereco.getNomeRua().isEmpty()) {
            throw new Exception("Nome da rua do endereço não informada");
        }

        if (endereco.getComplemento() == null || endereco.getComplemento().isEmpty()) {
            throw new Exception("Complemento do endereço não informado");
        }

        if (endereco.getDsBairro()== null || endereco.getDsBairro().isEmpty()) {
    ...
    public ArrayList<Endereco> findWithParameteres(int pessoaId)  throws Exception {
    
        return repository.findWithParameters(pessoaId);
